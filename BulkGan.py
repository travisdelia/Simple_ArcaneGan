...
def predict_fn(filepath, start_sec, duration, out_fps, interpolate):
    ...
    output_filepath = f"Arcane_{Path(filepath).stem}.mp4"        
    write_video(
        output_filepath,
        video_all,
        fps=fps,
...
def process_single_video(filepath):
    ...
    out_fps = 30
    interpolate_algorithm = 'Yes'
    predict_fn(filepath, start_sec, duration, out_fps, interpolate_algorithm)
    print(f"Process completed for video '{filepath}'. The output file is named 'out.mp4'.")
    ...
    for root, dirs, files in os.walk(dirpath):
        for file in files:
            if file.endswith('.mp4'):
                ...
                process_single_video(filepath)

def main_loop():
    video_path = input("Please enter the path to your video file or directory: ")
    if os.path.isdir(video_path):
        process_directory(video_path)
